<template>
	<div class="page_param">
		<div class="pc" v-if="obj.config.name">
			<mm_btn type="default-x" class="copy_btn btn-sm" @click.native="copy_file">复制文件</mm_btn>
			<mm_input title="请求路径" v-model="obj.config.path" @blur="set({ path: obj.config.path })"></mm_input>
			<mm_select title="请求方式" v-model="obj.config.method" :options="options_method" @change="set({ method: obj.config.method })"></mm_select>
			<mm_select title="返回类型" v-model="obj.config.contentType" :options="options_type" @change="set({ contentType: obj.config.contentType })"></mm_select>
...
<script>
	export default {
		...
		methods: {
			set(o) {
				o.name = this.query.name;
				this.$post(this.url_set + $.toUrl(this.query), o, function(json, status) {
					var res = json.result;
					if (res) {
						if (!res.bl) {
							$.toast(res.tip);
			...
			copy_file() {
				if (this.obj.param) {
					this.$copy(this.obj.param.filename)
				} else {
					this.$copy('参数配置文件不存在')
				}
			}
		...
		created() {
			var _this = this;
			this.$get(this.url_get_obj + $.toUrl(this.query), function(json, status) {
				if (json.result) {

 ...
 */
Encrypt.prototype.PKCS7Decode = function(buf) {
 ...
 */
Encrypt.prototype.PKCS7Encode = function(buf) {
 ...
 */
Encrypt.prototype.encode = function(msg) {
	...
	let appIdBuf = Buffer.from(this.appId) // 将 APPID 转成 buffer
 ...
 */
Encrypt.prototype.decode = function(encryptdMsg) {
...
Encrypt.prototype.genSign = function(timestamp, nonce, encrypt) {
	// 原始字符串
	let rawStr = [this.token, timestamp, nonce, encrypt].sort().join('');

	// 计算签名
	let signature = crypto.createHash('sha1').update(rawStr).digest('hex');

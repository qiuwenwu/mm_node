						...
						<mm_table type="2">
							...
							<tbody>
								<tr v-for="(o, idx) in list" :key="idx">
									...
									<td><span class="type">{{ get_value(list_type, o.type) }}</span></td>
									<td><span class="onoff">{{ avas[o.available] }}</span></td>
									<td><span class="onoff">{{ avas[o.hide] }}</span></td>
									<td><span class="onoff">{{ avas[o.can_comment] }}</span></td>
									<td><span class="name">{{ o.father_id }}</span></td>
									<td><span class="name">{{ o.city_id }}</span></td>
...
<style>
	/* 页面 */
	#mall_product_channel {}

	/* 表单 */
	#mall_product_channel .mm_form {}

	/* 筛选栏栏 */
	#mall_product_channel .mm_filter {}

	/* 操作栏 */
	#mall_product_channel .mm_action {}

	/* 模态窗 */
	#mall_product_channel .mm_modal {}

	/* 表格 */
	#mall_product_channel .mm_table {}

	/* 数据统计 */
	#mall_product_channel .mm_data_count {}
</style>

...
// SELECT * FROM `finance_stock_market` where `close` > 1000 GROUP BY `code`;
...
async function get_marketB(db) {
	var arr = await db.run('SELECT `code` FROM `finance_stock_market` where `close` > 1000 GROUP BY `code`;');
	await db.exec('DELETE from `finance_stock_market` where `close` > 1000;');
	var len = arr.length;
	console.log(`共${len}个股票`);
	for (var i = 0; i < len; i++) {
...
// http://money.finance.sina.com.cn/quotes_service/api/json_v2.php/CN_MarketData.getKLineData?symbol=sh000001&scale=60&ma=1&datalen=1023

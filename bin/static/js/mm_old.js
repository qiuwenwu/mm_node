...
+function ($) {
    ...
    $.hideIndicator = function () {
        ...
        if ($('.preloader-indicator-modal')[0]) return;
        $(defaults.modalContainer).append('<div class="preloader-indicator-overlay"></div><div class="preloader-indicator-modal">'+ htmlcontent +'</div>');
    };
    ...
    $.actions = function (params) {
        ...
        for (var i = 0; i < params.length; i++) {
            for (var j = 0; j < params[i].length; j++) {
                ...
                if (button.color) buttonClass += ' text-' + button.color;
...
+ function($) {
	...
	$(document).on('blur','.dropdown-toggle', function(){
		var id = '#' + $(this).attr('data-toggle');
		$(id).hide();
		});	
}(Zepto);